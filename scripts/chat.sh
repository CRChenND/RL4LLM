...
# Wrapper for interactive chat using infer/chat.py
# Usage:
#   bash scripts/chat.sh [ADAPTER_DIR] [extra args]
...
if [[ -n "${ADAPTER_DIR}" ]]; then
  ...
  if [[ -z "${ADAPTER_DIR}" ]]; then
    ...
    echo "Tip: use '--no-adapter' to chat with the base model only." >&2
    echo "Usage: bash scripts/chat.sh [outputs/your-run/final|--no-adapter] [--model_id ...] [--device ...]" >&2
    exit 1
  fi
fi
...
if [[ ${NO_ADAPTER} -eq 1 ]]; then
  python3 -m infer.chat "$@"
else
  python3 -m infer.chat --adapter "${ADAPTER_DIR}" "$@"
fi
